...
## 1.5 Development Environment Setup (Ubuntu VM)

> **Why this matters**: Cosmos SDK is Go-based, so `conda` does not apply (conda is for Python).
> Use Go/Node version managers to isolate this project's toolchain from other system installations.

### Option A: gvm + nvm + direnv (recommended for native VM use)

```bash
# 1. gvm (Go Version Manager)
sudo apt install -y bison build-essential
bash < <(curl -s -S -L https://raw.githubusercontent.com/moovweb/gvm/master/binscripts/gvm-installer)
source ~/.gvm/scripts/gvm

# 2. Install Go 1.21 and create a project-scoped pkgset
gvm install go1.21.5
gvm use go1.21.5 --default
gvm pkgset create blt-sand
gvm pkgset use blt-sand

# 3. nvm (Node Version Manager) for Ignite frontend scaffolds
curl -o- https://raw.githubusercontent.com/nvm-sh/nvm/v0.39.7/install.sh | bash
export NVM_DIR="$HOME/.nvm"
[ -s "$NVM_DIR/nvm.sh" ] && \. "$NVM_DIR/nvm.sh"
nvm install 20
nvm alias blt-sand 20

# 4. Ignite CLI
curl https://get.ignite.com/cli! | bash
sudo mv ignite /usr/local/bin/
ignite version  # verify v28.x or later

# 5. direnv for automatic activation on cd
sudo apt install -y direnv
echo 'eval "$(direnv hook bash)"' >> ~/.bashrc
source ~/.bashrc
```

Create `~/blt-sand/.envrc` so the environment activates automatically when entering the project directory:
...
Then once: `cd ~/blt-sand && direnv allow`.

### Option B: Docker (recommended if the VM hosts multiple projects)
...
A `Dockerfile` checked into the repo makes this reproducible:

```dockerfile
FROM golang:1.21
RUN apt-get update && apt-get install -y curl git make build-essential
RUN curl https://get.ignite.com/cli@v28.4.0! | bash && mv ignite /usr/local/bin/
WORKDIR /workspace
...
| Situation | Choose |
|-----------|--------|
| VM is dedicated to BLT-SAND | Option A (gvm + direnv) |
| VM hosts multiple Go projects | Option A with separate pkgsets per project |
| You want full isolation and easy teardown | Option B (Docker) |
| Cross-machine reproducibility matters | Option B (Docker) |
...
go version          # go1.21.5
node --version      # v20.x
ignite version      # v28.x
which go && which ignite  # paths should be under ~/.gvm or /usr/local
```
...
`X` (measured Phase 8, 2026-05-20): **≈ 135 kB** with the full member_table for N = 8,192 cluster members (12 cluster heads). Empty-payload blocks (no `MsgCommitSyncBlock`) measured at ~1.0 kB on-wire / 2.5 kB JSON over the RPC `/block` endpoint with 4 validator signatures, scaling to ~1.5–1.7 kB with 12 ed25519 commit signatures.
...
| Component | Estimated size | Measured (Phase 8, 2026-05-20) |
|-----------|----------------|--------------------------------|
| CometBFT header | ~400 B | ~600 B (observed via `/block` JSON, conservative) |
| LastCommit (4 ed25519 sigs in dev) | ~800 B for 12 | ~300 B for 4; project ~800 B for 12 |
| MsgCommitSyncBlock envelope | ~150 B | 271 B with empty member_table |
| BltBodyGlobal | ~14 B (varint) | ~14 B |
| BltClusterSummary × 12 | ~22 B × 12 = 264 B | matches estimate |
| BltMemberRecord × 8,192 | ~12 B × 8,192 ≈ 98 kB | **133.6 kB** (≈ 16.3 B/record after proto tags) |
| **Total per block** | **~99 kB** | **≈ 135 kB** for 8,192 members |

Sub-totals at smaller N from `TestSize_Sampled` (`x/blt/types/size_test.go`):

| members | MsgCommitSyncBlock wire size |
|---------|------------------------------|
| 0 | 271 B |
| 12 | 511 B |
| 256 | 4.40 kB |
...
Reason the measured value exceeds the estimate: each `BltMemberRecord`
encodes six fields, and proto3's per-field tag byte plus varint length
add ~1 byte per field beyond the raw payload width — closer to 16 B per
record than 12 B.
